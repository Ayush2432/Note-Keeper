...
firstly it would be an empty array*/
  ...
  const toggle = () => {
    // creating toggle function we r just changing the modal state
    setModal(!modal);
  };

  const pinTask = (obj, index) => {
    let tempList = taskList;
    tempList[index] = obj;
    localStorage.setItem("taskList", JSON.stringify(tempList));
    setTaskList(tempList);
...
5.pass this savetask function in the childcomponent so that child component can use this function*/
  ...
  return (
    // one div  for the header & other div for the task container
      ...
      <div className="task-container">
        {taskList &&
          taskList.map((obj, index) => (
            (index>=(count*5-5) && index<(count*5) && (obj.Pinned) &&
            <Card
              taskObj={obj}
              index={index}
